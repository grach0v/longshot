...
pub fn parse_vcf_potential_variants(
    ...
    for r in vcf.records() {
        ...
        if !bam_files_iteraction.chrom_to_tid.contains_key(&chrom) {
            eprintln!(
...
pub fn parse_vcf_potential_variants_multiple_bams(
    vcffile_name: &String,
    bamfile_names: &Vec<String>,
) -> Result<VarList> {
    // must assert that the VCF file is sorted correctly
    // can we just read it in and then check that it's sorted using the check_sorted function vs the bam's tlist?
    ...
    for bamfile_name in bamfile_names {
        for target in parse_target_names(bamfile_name)? {
            target_names.push(target);
        }
    }

    let bamfiles: Vec<bam::Reader> = 
                    bamfile_names.iter()
                    .map(|bamfile_name| bam::Reader::from_path(bamfile_name).chain_err(|| ErrorKind::BamOpenError))
                    .collect::<Result<_>>()?;

    let mut chrom2tid: HashMap<String, usize> = HashMap::new();

    let tmp = bamfiles[0].header().target_names().iter();

    let bam_target_names_iters: Vec<std::slice::Iter<&[u8]>> = 
                                bamfiles.iter()
                                .map(|bam| bam.header().target_names().iter())
                                .collect(); 


    for (t, name) in bam_target_names_iters.iter().flat_map(|it| it.clone()).enumerate() {
